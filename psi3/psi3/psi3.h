...
#pragma region setup
	...
	for (u64 i = 0; i < nParties; ++i)
	{
		if (i < myIdx)
		{
			...
			ep[i].start(ios, ip_array[myIdx].data(), port, false, name); //channel bwt i and pIdx, where i is sender
		}
		else if (i > myIdx)
		{
			...
			u32 port = 10200 + myIdx * 100 + i;						//get the same port; i=2 & pIdx=1 =>port=102
			ep[i].start(ios, ip_array[i].data(), port, true, name); //channel bwt i and pIdx, where i is receiver
		}
	}
