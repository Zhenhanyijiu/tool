...
namespace osuCrypto
{
  ...
  void *process_hash1_thread(void *arg)
  {
    ...
    for (auto i = 0; i < info->processNum; ++i)
    {
      ...
      H1.Reset();
      //对每一个y属于dataSetInput，映射成一个hash值，32字节（H1,H2）
      H1.Update(info->dataSetInputStart[i].data(),
                                 ...
                                 block *dataSetOutput)
  {
    ...
    for (int i = 0; i < numTh; i++)
    {
      ...
      infoArgs[i].dataSetInputStart =
          (vector<u8> *)(dataSetInput.data()) + i * stepLength;
      int fg = pthread_create(&threads[i], NULL, process_hash1_thread,
                              (void *)(infoArgs + i));
      if (fg)
      {
...
#pragma omp parallel for num_threads(numTh)
  ...
  int PsiReceiver::genPK0FromNpot(u8_t *pubParamBuf,
                                  const u64_t pubParamBufByteSize, u8_t **pk0Buf,
                                  u64_t *pk0BufSize)
  {
  ...
  int PsiReceiver::getSendMatrixADBuff(const u8_t *uBuffInput,
                                       const u64_t uBuffInputSize,
                                       const vector<vector<u8_t>> receiverSet,
                                       u8_t **sendMatrixADBuff,
                                       u64_t *sendMatixADBuffSize)
  {
...
#pragma omp parallel for num_threads(threadNum)
  ...
  // int PsiReceiver::getSendMatrixADBuff(const u8_t *uBuffInput, const u64_t
  // uBuffInputSize,
  //                                      const vector<vector<u8_t>> receiverSet,
  //                                      u8_t **sendMatrixADBuff, u64_t
  //                                      *sendMatixADBuffSize)
  // {
  //     int ublocksize = uBuffInputSize / sizeof(block);
  //     vector<block> uBuffInputBlock(ublocksize);
  ...
  //     }
  //     //生成encmsgoutput
  //     int fg = this->iknpOteSender.getEncMsg(uBuffInputBlock,
  //     this->encMsgOutput); if (fg != 0)
  //     {
  //         return fg;
  //     }
  //     //psi begin
  ...
  //     long start0 = start_time();
  //     transformInputByH1(commonAes, this->h1LengthInBytes, receiverSet,
  //     recvSet, this->threadNumOmp);
  //     // long useTimeH1 = getEndTime(timeCompute);
  //     printf("===>>计算H1用时:%ldms\n", get_use_time(start0));
  //     ////////// Transform input end //////////////////
  //     /*********for cycle start*********/
  //     printf("===>>widthBucket1(16/loc):%d,locationInBytes:%d\n", widthBucket1,
  //     locationInBytes); for (auto wLeft = 0; wLeft < this->matrixWidth; wLeft
  //     += widthBucket1)
  //     {
  //         auto wRight = wLeft + widthBucket1 < this->matrixWidth ? wLeft +
  //         widthBucket1 : this->matrixWidth; auto w = wRight - wLeft;
  //         //////////// Compute random locations (transposed) ////////////////
  //         commonPrng.get((u8 *)&commonKey, sizeof(block));
  //         commonAes.setKey(commonKey);
  //         for (auto low = 0; low < receiverSize; low += this->bucket1)
  //         {
  //             auto up = low + this->bucket1 < receiverSize ? low +
  //             this->bucket1 : receiverSize;
  //             //每256个输入处理一次，randomLocations==256blocks,Fk函数，Fk(H1(y))
  ...
  //                 // shift全1
  //                 // location >> 3(除以8)表示matrixDelta[i]的字节位置
  //                 // location & 0b0111,取出低3位；(location &
  //                 7)==0,1,2,3,4,5,6,7 matrixDelta[i][location >> 3] &= ~(1 <<
  //                 (location & 7));
  //             }
  //         }
  //         //////////////// Compute matrix A & sent matrix
  ...
  //             this->heightInBytes); for (auto j = 0; j < this->heightInBytes;
  //             ++j)
  //             {
  //                 // sentMatrix[i][j] ^= matrixA[i][j] ^ matrixDelta[i][j];
  ...
  void process_for_hash_map(HashMapParallelInfo *infoArg)
  {
    ...
    for (auto low = infoArg->startIndex; low < rightIndex;
  ...
  int PsiReceiver::genenateAllHashesMap()
  {
    ...
    for (int i = 0; i < threadNum; i++)
    {
      ...
      infoArgs[i].hashMap =
          (unordered_map<u64, std::vector<std::pair<block, u32_t>>>
...
#pragma omp parallel for num_threads(threadNum)
  ...
  //     }
  //     //接收集合中的每个元素
  //     for (auto low = 0; low < this->receiverSize; low +=
  //     this->bucket2ForComputeH2Output)
  //     {
  //         auto up = low + this->bucket2ForComputeH2Output < this->receiverSize
  //         ? low + this->bucket2ForComputeH2Output : this->receiverSize; for
  //         (auto j = low; j < up; ++j)
  //         {
  ...
  //             {
  //                 hashInputs[j - low][i >> 3] |=
  //                     (u8)((bool)(this->transHashInputs[i][j >> 3] & (1 << (j &
  //                     7))))
  //                     << (i & 7);
  //             }
  //         }
                                                   ...
                                                   vector<u32_t> *psiMsgIndex)
  {
    ...
    //
    u64 offset = 0;
    for (auto idx = 0; idx < up - this->lowL;
         ++idx, offset += this->hash2LengthInBytes)
    {
  ...
  ////PsiReceiver,接收对方发来的hash输出
  // int PsiReceiver::recvFromSenderAndComputePSIOnce(const u8_t *recvBuff, const
  // u64_t recvBufSize,
  //                                                  vector<u32_t> *psiMsgIndex)
  // {
  //     auto up = this->lowL + this->bucket2ForComputeH2Output < this->senderSize
  //     ? this->lowL + this->bucket2ForComputeH2Output : this->senderSize; if
  //     (recvBufSize != (up - this->lowL) * this->hash2LengthInBytes)
  //     {
  //         return -122;
  //     }
  ...
  //生成公共参数
  int PsiSender::genPublicParamFromNpot(u8_t **pubParamBuf,
                                        u64_t *pubParamBufByteSize)
  {
    return this->iknpOteReceiver.genPublicParamFromNpot(pubParamBuf,
                                                        pubParamBufByteSize);
  }
  ...
  int PsiSender::recoverMatrixC(const u8_t *recvMatrixADBuff,
                                const u64_t recvMatixADBuffSize)
  {
...
#pragma omp parallel for num_threads(threadNum)
  ...
  // int PsiSender::recoverMatrixC(const u8_t *recvMatrixADBuff, const u64_t
  // recvMatixADBuffSize)
  // {
  //     auto locationInBytes = (this->logHeight + 7) / 8;    // logHeight==1
  //     auto widthBucket1 = sizeof(block) / locationInBytes; // 16/1
  ...
  //     {
  //         // 256*1+4，每个元素为u8*，下面语句为创建一个u8*,260个u8
  //         transLocations[i] = new u8[this->senderSize * locationInBytes +
  //         sizeof(u32)];
  //     }
  ...
  //         {
  //             auto up = low + bucket1 < this->senderSize ? low + bucket1 :
  //             this->senderSize; commonAesFkey.ecbEncBlocks(this->sendSet + low,
  //             up - low, randomLocations); for (auto i = 0; i < w; ++i)
  //             {
  //                 for (auto j = low; j < up; ++j)
  //                 {
  //                     memcpy(transLocations[i] + j * locationInBytes,
  ...
  //                 for (auto j = 0; j < heightInBytes; ++j)
  //                 {
  //                     matrixC[i][j] ^= (recvMatrixADBuff + offset1 +
  //                     offset2)[j];
  //                 }
  //             }
  //             offset2 += this->heightInBytes;
  ...
  //         ///////////////// Compute hash inputs (transposed)
  //         ///////////////////// long startInput = start_time(); for (auto i =
  //         0; i < w; ++i)
  //         {
  //             // cout << "***********************33......" << endl;
  //             for (auto j = 0; j < senderSize; ++j)
  ...
  //                     shift;
  //                 this->transHashInputs[i + wLeft][j >> 3] |=
  //                     (u8)((bool)(matrixC[i][location >> 3] & (1 << (location &
  //                     7))))
  //                     << (j & 7);
  //             }
  //         }
  ...
  //计算本方的hash输出并发送给对方
  int PsiSender::computeHashOutputToReceiverOnce(u8_t *hashOutputBuff,
                                                 u64_t *sendBuffSize)
  {
